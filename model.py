...
class CBOWModel(nn.Module):
    ...
    def inner_forward(self, peripheral_words, center_words):
        """
        前向传播
        :param peripheral_words: 输入的词索引，形状为 (batch_size, context_size)批次和窗口大小
        :param center_words: 正样本标签，形状为 (batch_size,)这就是N个样本的中心词
        中国有很多经典的古诗词，假设窗口大小是5，那么就中心词就是古诗词中的每一个词，x就是每一个词的前后各两个词
        :return: 损失值
        """
        peripheral_words = self.embedding(peripheral_words)#将词索引映射为词向量，形状为 (batch_size, context_size, embedding_dim)
        peripheral_words = peripheral_words.mean(dim=1)  # 对上下文词向量取平均，形状为 (batch_size, embedding_dim)
        pos_emb = self.embedding(center_words)  # 获取正样本词向量，形状为 (batch_size, embedding_dim)
        #负采样
        #提取正样本标签
        pos_labels_np = center_words.cpu().numpy()
        #随机产生k个负样本标签
        neg_labes = random_neg_labels(center_words,
        ...
        neg_labes = torch.tensor(neg_labes, dtype=torch.long, device=peripheral_words.device)  # 转为Tensor
        #从参数中提取负样本词向量
        neg_emb = self.embedding(neg_labes)
        #计算正样本得分
        pos_scores = torch.sum(peripheral_words * pos_emb, dim=1, keepdim=True)  #[N,E] * [N,E] -> [N,1]
        #计算负样本得分
        neg_scores = torch.matmul(peripheral_words, neg_emb.t())  # 形状为 (batch_size, k) #[N,E] * [E,k] -> [N,k]
        #将正样本得分和负样本得分拼接在一起
    ...
    def forward(self, peripheral_words, center_words):
        loss = self.inner_forward(peripheral_words, center_words)
        return loss
...
class SkipGramModel(nn.Module):
    ...
    def inner_forward(self, center_words, peripheral_words):
        """
        前向传播
        :param center_words: 中心词索引，形状(batch_size,)
        :param peripheral_words: 正样本上下文词索引，形状(batch_size,)
        中国有很多经典的古诗词，假设窗口大小是5，那么就中心词就是古诗词中的每一个词，x就是每一个词的前后各两个词
        :return: 损失值
        """
        # 从同一嵌入矩阵中提取中心词和上下文词向量
        center_emb = self.embedding(center_words)  # (batch_size, embedding_dim)
        context_emb = self.embedding(peripheral_words)  # (batch_size, context_size, embedding_dim)#这里存在两种不同的上下文词向量，一种是多个词，一种是一个词，一个词的大小是(batch_size, embedding_dim)

        # 负采样
        # 提取正样本标签
        pos_labels_np = peripheral_words.cpu().numpy().reshape(-1) # 展平为(batch_size*window_size,)
        # 随机产生k个负样本标签
    ...
    def forward(self, peripheral_words, center_words):
        loss = self.inner_forward(center_words , peripheral_words)
        return loss
